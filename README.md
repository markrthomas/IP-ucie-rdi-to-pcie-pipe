...
└── docs/
    ...
    ├── uvm_verification.md                  # UVM architecture, scope, and closure plan
...
| `CHANGELOG.md` | Semantic version history (current release **v1.0.4**) |
...
| `make regress_nl1` | Optional: lint + **NUM_LANES=1** smoke (**assertions-only**, `obj_dir_nl1/`) |
| `test/uvm/` | Manual VCS/UVM 1.2 environment for TX-path UVM stimulus and future coverage closure |
...
The UVM environment lives under `test/uvm/` and is documented in `docs/uvm_verification.md`. It currently targets VCS/UVM 1.2 and focuses on TX-path smoke stimulus:
...
| UVM capability | Current status |
|----------------|----------------|
| Active RDI TX agent | Drives sequence items through `ucie_rdi_if` |
| Passive PIPE TX monitor | Captures accepted PIPE beats |
...
| Metric | Result | Notes |
|--------|--------|-------|
| **Line Coverage** | 95.80% overall | Generated by `make regress_cov` / `coverage.info` in this workspace; FIFO source is 100%, bridge source is 95.24% |
...
- **PIPE → RDI**: 3-4 cycles nominal (same FIFO CDC structure in reverse direction)
...
- **Data Paths**: Bidirectional transport: TX `RDI -> PIPE`, RX `PIPE -> RDI`

## Future Enhancements

1. **Protocol-specific sideband support**: Extend beyond data/error transport for additional UCIe or PIPE sideband semantics
...
GitHub Actions workflow `.github/workflows/verilator.yml` runs **`make regress`** (lint + Verilator smoke) on push/PR to `main` or `master`, then dependent jobs run **`make verilator_cov`** (optional **`coverage.info`** artifact) and **`make verilator_nl1`** (**NUM_LANES=1** smoke).
...
- **uvm_verification.md** — UVM architecture, component tables, sequence matrix, and closure gaps  
