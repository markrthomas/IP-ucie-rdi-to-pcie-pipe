...
- **UVM TX FIFO-full / backpressure closure**: Completed UVM closure plan item 3. The TX RDI monitor now publishes a cycle-accurate flow-control stream on a new `fc_ap` port that is *not* gated on the `valid & ready` handshake — required because `rdi_ready` and `rdi_flow_ctrl` are per-lane complements, so a full lane never produces a handshake beat and FIFO-full was previously unobservable. `ucie_rdi_fifo_full_seq` (24 all-lane beats with distinct incrementing per-lane payloads) runs concurrently with `pcie_pipe_deep_backpressure_seq` to drive every lane's TX FIFO to full. `cg_occupancy` is now fed from `fc_ap` (so its partial/all-lanes bins are reachable), and the scoreboard adds a `write_fc()` tracker plus an armed `expect_fifo_full` check that reports `SB_FIFO_FULL` unless all lanes reached full — with the existing per-lane queues proving in-order lossless drain once ready is released.
- **UVM RX randomized traffic and backpressure**: Completed UVM closure plan item 4. Added `pcie_pipe_rx_rand_seq` (12 constrained-random reverse-path beats) and `ucie_rdi_rx_backpressure_seq`, run concurrently in the sanity test so RX beats accumulate while `rdi_rx_ready` is stalled and drain when released — all validated by the existing mirrored RX scoreboard. RX backpressure is enabled by a new `ctrl` clocking block on `ucie_rdi_if` plus a `ucie_rdi_ready_driver`/sequencer added to the environment (vif provided from `uvm_test_top`), mirroring the existing PIPE TX ready-control path.
- **UVM width/occupancy/CRC functional coverage**: Completed UVM closure plan item 5. `ucie_rdi_pcie_coverage` now adds `cg_pipe_width` (RDI->PIPE width conversion: PIPE upper-half zero-extension plus lane-0 payload magnitude buckets), `cg_occupancy` (`$countones` of RDI `flow_ctrl` as an observable proxy for elastic-buffer occupancy), and `cg_crc` (CRC enable/error cross). CRC status is routed to the TX PIPE monitor through new observation-only `crc_enable`/`crc_error` signals on `pcie_pipe_if`, driven from `uvm_test_top`. The coverage report now prints WIDTH/OCC/CRC percentages alongside the direction covergroups.
...
- **UVM ready-control hold constraint**: Widened `pcie_pipe_ready_transaction`'s `c_hold_cycles` from `[1:32]` to `[1:1024]`. The backpressure sequences request equality holds (e.g. `hold_cycles == 48`) via `randomize() with {}`, which *narrows* rather than overrides the class constraint — so the previous `[1:32]` bound made those randomizations infeasible (and would fail under VCS). The wider bound also covers the new deep-fill holds.
